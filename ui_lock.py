...
def MP_TQDM(func, lock, shared=[], args=[], sz=1, pc=2, reset=True):
    def chunks(l, n):
        for i in range(0, len(l), n):
            yield (pc, shared, l[i:i + n])
    rets = []
    with multiprocessing.Pool(pc, initializer=init, initargs=(l,), maxtasksperchild = 1 if reset else None) as p:
        # The master process tqdm bar is at Position 0
        with tqdm(total=math.ceil(len(args)/sz), desc="total", leave=False) as pbar:
...
def MP_TQDM_WORKER(func):
    ...
    def d_func(*args, **kwargs):
        ...
        rets = []
        desc = str(worker_id)
        with tqdm(argset, desc=desc, position=worker_id, leave=False) as pbar:
            for arg in argset:
...
if __name__ == "__main__":
    ...
    N = 99999000
    M = 4
    ANS = MP_TQDM(myfunc, l, shared=[M], args=range(1, N+1), sz=100000, pc=16, reset=True)
